...
def plot_leak_rounds(observed_bits, times, threshold=100, output_file='leak_plot_Q.pdf'):
    ...
    ax.plot(rounds, times, color='#1f77b4', linewidth=2.5, marker='o',
            markerfacecolor='white', markeredgecolor='#1f77b4', markersize=6, label=f"DOLMA-Conservative (M+R)", zorder=3)
    ...
    # Ensure 120 is on the y-axis ticks
    y_min = min(times)
    y_max = max(times)
    buffer = 40
    yticks = sorted(set([threshold] + list(range(y_min, y_max + buffer, 40))))
...
if __name__ == "__main__":
    observed_bits, times = parse_attack_log('x86arithmetic.log')
    if len(observed_bits) == 8:
        plot_leak_rounds(observed_bits, times, threshold=105)
    else:
