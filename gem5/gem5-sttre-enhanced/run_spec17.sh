...
GEM5_DIR=${GEM5_DIR:-$SCRIPT_DIR}                         # Install location of gem5
SPEC_DIR=${SPEC_DIR:-/home/chengxiaoyu/spec20171.1.9}         # Install location of your SPEC2017 benchmarks


# Get command line input. We will need to check these.
BENCHMARK=$1                    # Benchmark name, e.g. bzip2
SCHEME_NAME=${SCHEME_NAME:-${2:-default}}
...
if [[ -e "$WORK_DIR" ]]; then
    echo "Work directory $WORK_DIR already exists; choose a fresh WORK_ROOT."
    exit 1
fi
mkdir -p "$WORK_DIR" "$OUTPUT_DIR"
cp -a --reflink=auto "$RUN_DIR"/. "$WORK_DIR"/
...
echo "WORK_DIR:                                     $WORK_DIR" | tee -a $SCRIPT_OUT
echo "SCHEME_NAME:                                  $SCHEME_NAME" | tee -a $SCRIPT_OUT
...
echo ""
echo "Original SPEC benchmark runtime directory: $RUN_DIR" | tee -a $SCRIPT_OUT
echo "Changing to isolated benchmark runtime directory: $WORK_DIR" | tee -a $SCRIPT_OUT
cd "$WORK_DIR"
...
# DEBUGFLAGS= "--debug-flags=IQ,IEW,Fetch,LSQUnit,Commit,ROB \
# --debug-file=trace.out" 

DEFAULT_BOOM_FLAGS="--num-cpus=1 \
--bp-type=BiModeBP \
--caches \
--l2cache \
--num-l2cache=1 \
...
# UnsafeBaseline --needsTSO=0 --STT=0 --implicit_channel=0 --moreTransmitInsts=0
# Spectre --needsTSO=0 --STT=1 --implicit_channel=1 --moreTransmitInsts=2
# Spectre-SilentBranch --needsTSO=0 --STT=1 --implicit_channel=1 --moreTransmitInsts=2
# Spectre-M+R --needsTSO=0 --STT=1 --implicit_channel=1 --moreTransmitInsts=2 
# Conservative-M+R --needsTSO=0 --STT=1 --implicit_channel=1 --moreTransmitInsts=2

# Actually launch gem5!
$GEM5_DIR/build/X86/gem5.opt \
--outdir=$OUTPUT_DIR/m5out \
$GEM5_DIR/configs/spec2017/se_spec2017.py \
$BOOM_FLAGS \
--benchmark=$BENCHMARK \
--benchmark_stdout=$OUTPUT_DIR/$BENCHMARK.out \
--benchmark_stderr=$OUTPUT_DIR/$BENCHMARK.err  | tee -a $SCRIPT_OUT
